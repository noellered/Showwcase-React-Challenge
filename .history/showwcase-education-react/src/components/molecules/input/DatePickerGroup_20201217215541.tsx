...
const DatePickerGroup:FunctionComponent<DatePickerGroupProps> = ({ type, handleChange }) => {
    ...
    return(
        ...
        <div className={classes.inputGroup}>
            <MonthPicker className={`${classes.input} ${classes.mr}`} id="`${startMonth}`" onChange={(e)=> setState({...state, month: e.target.value})}/>
            <YearPicker className={`${classes.input} ${classes.mr}`} id="startYear" onChange={(e)=> setState({...state, year: e.target.value})}/>
        </div>
        </Grid>
    )
}
